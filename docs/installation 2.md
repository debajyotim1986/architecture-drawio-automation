...
git clone <your-fork-or-this-repo-url> AI_Atlassian_copilot
cd AI_Atlassian_copilot
```
...
1. Open the workspace folder (`AI_Atlassian_copilot`) in VS Code.
...
4. Click the **MCP servers** indicator (in the chat input area). You
   ...
   reveal **10 tools**: `list_diagrams`, `read_diagram`,
   `read_diagram_summary`, `create_diagram`, `add_node`, `add_edge`,
   `update_node`, `remove_element`, `list_stories`, `read_story`.
...
1. Copy [`examples/sample-jira-story.txt`](../examples/sample-jira-story.txt)
   into [`jira-stories/`](../jira-stories/):
   ```bash
...
4. Open the resulting `diagrams/order-notification.drawio` file — the
...
(`AI_Atlassian_copilot/drawio-mcp-server/`) with the venv activated
...
rm diagrams/order-notification.drawio               # start over for one diagram
```
...
- VS Code Copilot Chat lists `drawio` as **connected** with 10 tools.
